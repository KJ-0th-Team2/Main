...
class JungleNav extends HTMLElement {
  ...
  _openModal(type) {
    ...
    card.innerHTML = type === 'login' ? this._loginTemplate() : this._signupTemplate();
    overlay.classList.remove('hidden');
    document.body.style.overflow = 'hidden';

    card.querySelector('#jn-close-btn').addEventListener('click', () => this._closeModal());

    if (type === 'login') {
      card.querySelector('#jn-submit-btn').addEventListener('click', () => this._handleLogin());
      card.querySelector('#jn-switch-link').addEventListener('click', () => this._openModal('signup'));
    } else {
      card.querySelector('#jn-submit-btn').addEventListener('click', () => this._handleSignup());
      card.querySelector('#jn-switch-link').addEventListener('click', () => this._openModal('login'));
    }
  }
  ...
  _signupTemplate() {
    return `
      ...
      <div class="flex flex-col gap-3 mb-5">
        <input id="jn-su-id" type="text" placeholder="아이디"
          class="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm text-gray-800
                 placeholder-gray-400 bg-gray-50 focus:outline-none focus:border-gray-800
                 focus:ring-2 focus:ring-gray-800/10 transition-all duration-150" />
        <input id="jn-su-pw" type="password" placeholder="비밀번호"
          class="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm text-gray-800
                 placeholder-gray-400 bg-gray-50 focus:outline-none focus:border-gray-800
                 focus:ring-2 focus:ring-gray-800/10 transition-all duration-150" />
  ...
  _handleSignup() {
    ...
    if (!id || !pw || !student) { alert('모든 항목을 입력해주세요.'); return; }

    this.dispatchEvent(new CustomEvent('jungle-signup', {
